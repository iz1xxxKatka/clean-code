...
Текст, окруженный с двух сторон ~, должен помещяться в HTML-тег <img src="..." />.
Пример: ~какой-то_текст~ <=> <img src="какой-то_текст"/>.
Описание ~текста~: 
	1) текст не может быть пустым;
	2) в тексте не должно быть пробелом и переносов строк;
	2) перед началом маркировки должен стоять пробел или начало строки, в противном случае останется символ ~;
	3) после конца маркировки должен стоять пробел или конец строки, в противном случае останется символ ~.